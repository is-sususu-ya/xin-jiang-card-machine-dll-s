...
#ifdef linux
#define sock_connect_tou(host, port) sock_connect0(host, port)
#endif
...
#endif
{
	...
	for (; !hALB->m_nQuit;)
	{
		...
		if (hALB->m_nInterface == INTERFACE_NET)
		{
			...
			nsel = select(hALB->m_nSocket + 1, &rfd_set, NULL, NULL, &tv);
...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
	{
		printf("usage: %s 192.168.1.123 192.168.1.123 \r\n", argv[0]);
		return 0;
	}
	while (1)
	{
		...
		case 'a':
			...
			alb2 = DEV_Open(argv[2]);
			printf("结束连接，时长：%d \r\n", time(NULL) - tick);
			if (alb2)
			{
				printf("连接成功！\r\n");
				DEV_SetEventHandle(alb2, onAlbEvent);
			}
			else
			{
				printf("连接失败！\r\n");
