...
__int64 GetSystemTime64()			// get system time (UTC) in msec
{ 
#ifdef linux
	static __int64 begin_time = 0;
	__int64 now_time;
	struct timespec tp;
	UINT64 tmsec = 0;
...
DLLAPI BOOL CALLTYPE  TCR8_CloseDevice(TCR8HANDLE h)
{
	...
	for(i=0; i<10 && h->m_hThread; i++ ) Sleep(5);
		if ( h->m_hThread )
			TerminateThread( h->m_hThread, 0 );
	h->m_hThread = NULL;
	DeleteObject( h->m_hMutex );
...
////////////////////////////////////////////////////////////////////////////////////////
/*
 * This function read a comment message from TCR8 communication line.
 * A comment message will be terminated on one of following conditions:
 *  - Normal terminated be a '}' character
 *  - Time space between next character longer then 10 msec (character interval time out)
 *  - A new protocol packet header (first 3 bytes) is encounted. Next protocol packet will be read also
 *	  and stored in 'NextFrame'. Work thread main loop will handle both frame (comment and protocol)
 *  - COM port I/O Error (return with empty comment message "{}"
 *
 *	On return, 'buf' is holding the message of comment message (complete one or incompleted), 
 *  while 'NextFrame' is holding the protocol frame which terminates the comment message (when there is one)
 *  return value is always the length of message: strlen(buf)
 */
#define MSG_CTL_ALL		"ABCDEFGHV01"

static int ReadCommentPacket( TCR8HANDLE h, char *buf, int size, char *NextFrame, int *szNextFrame )
{
	char	*ptr = buf+1;
	int		ch;
	...
	if( _IsConnectWithCom(h) )
		...
		fdSocket = h->m_sockTCP;

	NextFrame[0] = '\0';
	*szNextFrame = 0;

#ifdef linux
	while ( (ptr-buf)<size && ( _IsConnectWithCom(h) ? (ch=tty_getc(tty,100, 0)) >= 0 : (ch=sock_getc(fdSocket, 100)) >= 0 ) )
#else
	while ( (ptr-buf)<size && ( _IsConnectWithCom(h) ? (ch=tty_getc(tty,100)) >= 0 : (ch=sock_getc(fdSocket, 100)) >= 0 ) )
#endif
	{
		*(ptr++) = ch;
		if ( ch==MSG_ETX1 )
		...
		{
			// Check for a begining of next protocol message (MSG_ETX1 could be lost some how)
			bNewStart = TRUE;
			nHead = 0;
			NewHead[nHead++] = ch;
		}
		else
		{
			...
			{
				NewHead[nHead++] = ch;
				if ( nHead==3 && isdigit(NewHead[1]) && strchr(MSG_CTL_ALL,NewHead[2])!=NULL )
				{
				...
				else if ( (nHead==2 && !isdigit(ch)) || (nHead==3 && strchr(MSG_CTL_ALL,ch)==NULL) )
				{
					// Does not match protocol header requirement -- reset bNewStart
					bNewStart = FALSE;
				}
			}
		}
...
RetryConnect:
	...
	for( ;!h->m_bQuit; )
	{
		...
		if( !_IsConnectWithCom(h) )
		{
			...
			{
				TRACE_LOG(h, "[WINSOCK ERROR] select error code = %d\n", WSAGetLastError());
				sock_close( h->m_sockTCP );
				h->m_sockTCP = INVALID_SOCKET;
				goto RetryConnect;
			}
...
#ifdef linux
		if ( _IsConnectWithCom(h) ? (ch=tty_getc( h->m_tty, tout, 0 )) >=0 : (ch=sock_getc(h->m_sockTCP, tout)) >= 0 )
#else
		if ( _IsConnectWithCom(h) ? (ch=tty_getc( h->m_tty, tout )) >=0 : (ch=sock_getc(h->m_sockTCP, tout)) >= 0 )
#endif
		{ 
			if ( ch == MSG_STX )
			{
			...
			{
				buf[0] = ch;
				rlen = ReadCommentPacket( h, buf, sizeof(buf)-1, NextFrame, &szNextFrame );
			}
			else
			{ 
				skipBuf[nSkip++] = ch; 
				if( nSkip >= (sizeof(skipBuf) - 1) )
				{
					...
					{
						TRACE_LOG(h, "[Protocol content error] throw away %d bytes: %s\n", nSkip, skipBuf );
					}
					nSkip = 0;
				}
				continue;
			...
			if ( buf[0] == MSG_STX1 )
			{
				if ( rlen > 2 )
				{
					...
					TRACE_LOG(h,buf);
					buf[rlen-2] = '\0';
					TriggerOnKernelLog( h, buf+1);	// every thing between { and }, and null terminated.
				} 
				if ( szNextFrame > 0 )
				{
					memcpy( buf, NextFrame, szNextFrame );
					buf[szNextFrame] = '\0';
					rlen = szNextFrame;
					szNextFrame = 0;
					NextFrame[0] = '\0';
			...
			if ( (ptr=strrchr( buf, MSG_STX )) > buf )
			{
				nSkip = (int)(ptr - buf);
				memcpy( skipBuf, buf, nSkip );
				skipBuf[ nSkip ] = '\0';
				rlen -= nSkip;
				memmove( buf, ptr, rlen+1 );		// include null character on end
				sprintf(msg, "[抛弃%d个字节] %s\n", nSkip, skipBuf );
				TRACE_LOG( h, msg );
				nSkip = 0;
			} 
			if ( IsValidPacket( h, buf ) )
			{
				...
				if ( IsAckPacket( h, buf ) )
					...
					NakedPacket( h, /*buf[1]*/packet_index - '0' );
				else
				{
					if ( IsStatusPacket( h, buf ) )
					{
						// time before we declare machine is offline
						// if we are waiting for a packet to establish link, now is the time to go
						if ( !h->m_bOnline )
						{
		...
		else if ( ch == -2 && _IsConnectWithCom(h) )
		{
			// TTY I/O Error, Close it
			if( 1/*IsChinese()*/ )
			{
				TRACE_LOG(h,"[COMERR] 串口I/O错误 (%d). 关闭串口结束线程!\n", GetLastError() );
			}
			else
			{
				TRACE_LOG(h,"[COMERR] COM I/O Error (%d). Close COM and end thread!\n", GetLastError() );
			}
			tty_close( h->m_tty );
			h->m_tty = -1;
		...
		if ( t_now > h->m_tHeartBeat2Recv && h->m_bOnline )
		{
			...
			// report TCR8 offline, wait for a status packet to regain connection.
			ReportOnConnection( h, FALSE );
		}

		/* check for time to sync system clock with machine */
...
static int SendAck( TCR8HANDLE h, char cseq )
{
	...
	if ( '0' <= cseq && cseq <= '9' )
	{
		...
		if ( (_IsConnectWithCom(h) ? tty_write( h->m_tty, cAck, len/*4*/ ) : sock_write(h->m_sockTCP, cAck, len) ) == len/*4*/ )
		{
...
static int SendNak( TCR8HANDLE h, char cseq )
{
	...
	if ( '0' <= cseq && cseq <= '9' )
	{
		...
		if ( (_IsConnectWithCom(h) ? tty_write( h->m_tty, cNak, len/*4*/ ) : sock_write(h->m_sockTCP, cNak, len)) == len/*4*/ )
		{
	...
	if ( 0 <= nSeq && nSeq < 10 && h->_laneMsg[ nSeq ].msg_state == MST_WAITACK )
	{
		...
		for( i=0; i<10; i++ )
		{
			...
			if (h-> _laneMsg[ nSeq ].msg_state == MST_WAITSEND )
			{
				...
				if( _IsConnectWithCom(h) )
					tty_write( h->m_tty, h->_laneMsg[ nSeq ].msg_body, h->_laneMsg[ nSeq ].msg_len );
				else
					sock_write(h->m_sockTCP, h->_laneMsg[ nSeq ].msg_body, h->_laneMsg[ nSeq ].msg_len);
...
*/
	...
	if ( packet[0] == MSG_STX && packet[len-1] == MSG_ETX && packet_index >= '0' && packet_index <= '9' )
	{
		...
		for( i=0; i<sizeof(MsgCodeLen) / sizeof(struct msg_codelen); i++ )
		{
			bLen =    (!_IsHuaNanKuaiSuProtocol(h) && (MsgCodeLen[i].len == len  )) 
				   || ( _IsHuaNanKuaiSuProtocol(h) && (MsgCodeLen[i].len == len-1));
		...
		if ( h->_laneMsg[i].msg_state == MST_WAITACK )
		{
			if ( tmsNow >= h->_laneMsg[i].t_resend )
			{
				if( _IsConnectWithCom(h) )
					...
					sock_write( h->m_sockTCP, h->_laneMsg[ i ].msg_body, h->_laneMsg[ i ].msg_len );
