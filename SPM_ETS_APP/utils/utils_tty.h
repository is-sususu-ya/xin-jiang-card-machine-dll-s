...
extern "C"
{
	...
	/* tty_open - open a linux tty device for serial I/O communication
	 * device: TTY device file name like "/dev/ttyS0"
	 */
	...
	/*  raw mode control
	 * tty_raw - set tty as raw mode at specified baudrate, databit and parity
	 * tty_setraw - only place specified tty as raw mode without changing any line setting.
	 *   fd_tty: the fd for a tty.
	 *   baud:   the baud rate setting. 0 means do not change the orignal line speed.
	 *   databit: 5, 6, 7, 8
	 *   parity: 0 none, 1 odd, 2 even
	 */
	int tty_raw(int fd_tty, int baud, int databit, int parity);
	int tty_setraw(int fd_tty, struct termios *ios);

	/*
	 * tty_get: get current tty device termios.
	 * tty_set: set the tty device termios. Ususally, restore tty setting get from tty_get
	 */
	void tty_get(int fd_tty, struct termios *p_ttyios);
	void tty_set(int fd_tty, struct termios *p_ttyios);
...
// pseudo functions
#define tty_write(tty, buf, len) write(tty, buf, len)
#define tty_close(tty) close(tty)
