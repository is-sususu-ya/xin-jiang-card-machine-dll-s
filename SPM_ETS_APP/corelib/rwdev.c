...
{
	int ret = 0;
	static int64_t ltLastTrigger = 0;
	...
	case QR_EVT_READY:
		...
		// LED_SoundDu();
		// usleep(400 * 1000);
		// led_voice_send(2, "扫码成功，正在支付");
#endif

		break;

	case QR_EVT_KEYPRESS:

		ltrace("收到来自费显转发的求助按键\r\n");
		if (ltLastTrigger < get_sys_tick64())
		{
			ltLastTrigger = get_sys_tick64() + 5000;
...
{
	Core_led_callback(NULL, QR_EVT_READY, msg);
	Core_led_callback(NULL, QR_EVT_KEYPRESS, msg);
}
...
static int CORE_load_modules()
{
	...
	if (load_module_by_name("./mod_qr.so") == 0)
	{
		...
		if (ioModule)
		{
			...
			// strcpy(buf, "dev=\"192.168.1.99\"");
			sprintf(buf, "dev=\"%s\"", g_apconfig.qr_scan_up);
...
void Qr_Ctrl(int opt)
{
	...
	int type = get_module_type("德亚扫码枪");
	cIOModule *ioModule = NULL;
	ioModule = (cIOModule *)CORE_find_module(type, NULL);
	if (ioModule)
	{
		ioModule->qr_ctrl(opt == 0 ? 0 : 1);
	}
	else
	{
		ltrace("扫码模块加载失败!r\n");
...
static void load_config()
{
	...
	valstring = ConfigGetKey("qr_scan_up");
	if (valstring)
		strcpy(g_apconfig.qr_scan_up, valstring);

	valstring = ConfigGetKey("robot_hand");
	if (valstring)
		strcpy(g_apconfig.robot_hand, valstring);
