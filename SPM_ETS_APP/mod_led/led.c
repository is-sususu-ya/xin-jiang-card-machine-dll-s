...
static int led_getc(OBJECT_S *pobj)
{
    int chr;
    if (pobj->work_mode == WORK_TTY)
    {
        chr = tty_getc(pobj->fd, 10, 0);
    }
    else
    {
        chr = msock_getc(pobj->fd, 10);
    }
   // printf("chr:%x\r\n", chr);
    return chr;
}

static int led_get_n_bytes(OBJECT_S *pobj, uint8_t *buf, int len)
{
    int chr = 0;
    if (pobj->work_mode == WORK_TTY)
    {
        chr = tty_gets(pobj->fd, buf, len, 10, 0, NULL);
    }
    else
    {
        chr = sock_read_n_bytes_tout(pobj->fd, buf, len, 10);
    }
   // printf("hex[%d]:%s\r\n", chr, show_hex(buf, chr));
    return chr;
}

static int get_packet(OBJECT_S *pobj, char *buf, int maxlen)
{
    int index = 0;
    int len;
    int tlen = -1;
...
get_hdr:
    ...
    if (chr == -1)
        goto error_return;
    if (chr != 0xa1 && chr != 0xf1 && chr != 0xe3 && chr != 0xe5)
        goto get_hdr;
    buf[0] = chr;
    if (chr == 0xa1)
    {
        len = led_get_n_bytes(pobj, buf + 1, 6);
        tlen = len;
        if (len < 6)
            goto error_return;
        len = ((buf[5] & 0xf0) >> 4) * 1000 + (buf[5] & 0xf) * 100 + ((buf[6] & 0xf0) >> 4) * 10 + (buf[6] & 0xf);
        len++;
        rlen = led_get_n_bytes(pobj, buf + 7, len);
        tlen = 7 + rlen;
        if (rlen < len)
            goto error_return;
        return len + 7;
    ...
    {
        index = 1;
        while ((chr = led_getc(pobj)) != -1 && chr != 0xFF && chr != 0x0a)
            buf[index++] = chr;
        tlen = index;
        if (chr == -1)
            goto error_return;
    ...
    return len;
error_return:
    printf("recv frame error[%d][%s]..\r\n", tlen, tlen > 0 ? show_hex(buf, tlen) : "");
    return -1;
}
...
static void *work_thread(void *arg)
{
    ...
    while (pHvObj->run)
    {
        ...
        {
            recv_len = get_packet(pHvObj, (char *)recv_buf, sizeof(recv_buf));
            ...
            else
            {
              //  printf("shw:%s\r\n", show_hex(recv_buf, recv_len));
                if (2 == recv_len)
                {
                    if (recv_buf[0] == 0xE3 && recv_buf[1] == 0xE4)
                    {
                        if (call_back)
                            call_back(EVT_IOCHANGE, recv_buf);
                    }
                    if (recv_buf[0] == 0xE5 && recv_buf[1] == 0xE6)
                    {
...
{
    OBJECT_S *pobj = (OBJECT_S *)h;
    unsigned char dat[40] = {"[v3][x1]sound118"};
    dat[2] = vol + '0';
